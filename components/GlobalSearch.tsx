...
export function GlobalSearch() {
  ...
  return (
    <>
      <Button 
        variant="outline" 
        className={`relative h-12 w-full justify-start rounded-xl bg-slate-50/80 dark:bg-slate-900/80 hover:bg-white dark:hover:bg-slate-950 text-base font-normal text-muted-foreground shadow-sm hover:shadow-md transition-all sm:pr-14 md:w-full lg:w-[500px] border-slate-200 dark:border-slate-800`}
        onClick={() => setOpen(true)}
      >
        <span className="inline-flex items-center gap-3">
           <Search className="h-5 w-5 text-primary" />
           <span className="text-slate-500 dark:text-slate-400 font-medium">{dictionary.navbar.searchPlaceholder}</span>
        </span>
        <kbd className={`pointer-events-none absolute ${locale === 'he' ? 'left-[0.4rem]' : 'right-[0.4rem]'} top-[0.4rem] hidden h-8 select-none items-center gap-1 rounded-lg border dark:border-slate-700 bg-white dark:bg-slate-950 px-2.5 font-mono text-[12px] font-bold text-slate-500 dark:text-slate-400 opacity-100 sm:flex shadow-sm`}>
